...
namespace PWH.Grids.Pathfinding
{
    public class Pathfinder : BasicSingleton<Pathfinder>
    {
        ...
        public List<IPathfindingNode> Init(Grid<IPathfindingNode> graph,IPathfindingNode startNode,IPathfindingNode goalNode,bool DEBUG_showPath = false,float DEBUG_duration = 10f)
        {
            ...
            if (DEBUG_showPath)
            {
                ...
                foreach (IPathfindingNode cell in pathNodes)
                {
                    ...
                    Debug.DrawLine(graph.GetWorldPosition(x, y,true), graph.GetWorldPosition(_x, _y, true), DEBUG_Colour, DEBUG_duration, false);
                }
            }
        ...
        void ExpandFrontierBFS(IPathfindingNode node)
        {
            ...
            for (int i = 0; i < node.neighbours.Count; i++)
            {
                ...
                float distance = graph.GetCellDistance(node.xIndex, node.yIndex, n.xIndex, n.yIndex);
        ...
        void ExpandFrontierAStar(IPathfindingNode node)
        {
            ...
            for (int i = 0; i < node.neighbours.Count; i++)
            {
                ...
                if (!frontierNodes.Contains(n))
                {
                    float distanceToGoal = graph.GetCellDistance(node.xIndex, node.yIndex, goalNode.xIndex, goalNode.yIndex);
